...
namespace Grapevine
{
    ...
    public class ContentType
    {
        ...
        static ContentType ()
        {
            ...
            var contentTypes = typeof(ContentType).GetFields(BindingFlags.Public|BindingFlags.Static)
                .Select(f => f.GetValue(null))
                .Where(f => f.GetType() == ct)
                .Cast<ContentType>()
                .ToList();

            foreach (var contentType in contentTypes) _contentTypes.Add(contentType, contentType);
        }
